    ...
    <div class="panel-body stack-lg">
      <div class="sub-panel">
        <div class="field-headline">
          ...
          <label class="toggle">
            <input :checked="subtitleEnabled" type="checkbox" @change="$emit('update:subtitleEnabled', $event.target.checked)" />
            <span></span>
          </label>
        </div>
        <p class="helper-text">{{ subtitleStatus }}</p>

        <div class="stack-md">
          <label class="field">
            <span class="field-label">字幕样式</span>
            <select class="select-input" :value="subtitleStyle" @change="$emit('update:subtitleStyle', $event.target.value)">
          ...
          <label class="field">
            ...
            <select class="select-input" :value="subtitlePosition" @change="$emit('update:subtitlePosition', $event.target.value)">
      ...
      <div class="sub-panel">
        ...
        <div class="stack-md">
          <label class="field">
            ...
            <select class="select-input" :value="selectedBgm" @change="$emit('update:selectedBgm', $event.target.value)">
          ...
          <label class="field">
            ...
            <input class="range-input" type="range" min="0" max="100" :value="narrationVolume" @input="$emit('update:narrationVolume', Number($event.target.value))" />
          </label>
          ...
          <label class="field">
            ...
            <input class="range-input" type="range" min="0" max="100" :value="bgmVolume" @input="$emit('update:bgmVolume', Number($event.target.value))" />
          </label>
        </div>
      </div>
      ...
      <div class="sub-panel">
        ...
        <div class="control-grid">
          <label class="field">
            ...
            <select class="select-input" :value="selectedAspect" @change="$emit('update:selectedAspect', $event.target.value)">
          ...
          <label class="field">
            ...
            <select class="select-input" :value="selectedQuality" @change="$emit('update:selectedQuality', $event.target.value)">
        ...
        <label class="field">
          ...
          <input :value="outputPath" class="text-input" type="text" @input="$emit('update:outputPath', $event.target.value)" />
        </label>

        <label class="checkbox-row">
          <input :checked="saveToLibrary" type="checkbox" @change="$emit('update:saveToLibrary', $event.target.checked)" />
          <span>保存到素材库</span>
        </label>

        <label class="checkbox-row">
          <input :checked="exportMaterials" type="checkbox" @change="$emit('update:exportMaterials', $event.target.checked)" />
          <span>导出素材包</span>
        </label>

        <button class="primary-button primary-button--full" type="button" :disabled="true" @click="$emit('render-video')">
          成片导出待接入
        </button>
      </div>
    </div>
