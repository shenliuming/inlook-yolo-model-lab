...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        ...
        sample_rate: int
        try:
            waveform_array, sample_rate = sf.read(str(resolved_path), dtype="float32", always_2d=True)
            waveform_tensor = torch.from_numpy(waveform_array.T.copy()).to(torch.float32)
        except Exception:
            waveform_tensor, sample_rate = torchaudio.load(str(resolved_path))
        ...
        current_channels = int(waveform_tensor.shape[0])
        if current_channels == target_channels:
            pass
        elif current_channels == 1 and target_channels > 1:
            waveform_tensor = waveform_tensor.repeat(target_channels, 1)
        elif current_channels > 1 and target_channels == 1:
        ...
        return waveform_tensor.unsqueeze(0).detach().cpu().numpy().astype(np.float32, copy=False)
