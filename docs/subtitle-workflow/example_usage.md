...
```bash
uv run python apps/yolo-api/services/subtitle_tool/check_env.py
```

## 2. 视频原声直接打字幕

```bash
uv run python apps/yolo-api/services/subtitle_tool/subtitle_pack.py \
  --video ./input.mp4 \
...
```bash
uv run python apps/yolo-api/services/subtitle_tool/burn_subtitles.py \
  --video ./input.mp4 \
